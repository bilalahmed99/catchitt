        ...
        <div key={key1} className={style.comment}>
            <div className={style.commemtHeader}>
                <div>
                    ...
                    <p style={{ fontSize: 12 }} className={style.cheaderText}>
                        {timeConverter(data.createdTime)}
                    </p>
                </div>
                <div>
                    <img src="../../../../public/images/icons/commentSec/Heart.svg" alt="" />
                    <p style={{ fontSize: 14 }} className={style.cheaderText}>
                        {data.likes}
                    </p>
                </div>
            </div>
            <div className={style.commentH}>
                ...
                {!viewReplys && data.replies.length > 0 ? (
                    <div onClick={() => setviewReplys(true)}>
                        <p className={style.replyBtn}>View replies ({data.replies.length})</p>
                        <img
                            style={{ cursor: 'pointer' }}
                            src="../../../../public/images/icons/commentSec/ArrowDown.svg"
                            alt=""
                        />
                    </div>
                ) : (
                    <span
                        ...
                        onClick={() => replyBtn(data.user.name, data.id)}
                    >
                        Reply
                    </span>
                )}
            ...
            {viewReplys
                ? data.replies.map((reply: any, i: number) => (
                      <ReplyComment key2={i} reply={reply} />
                  ))
                : null}
            <hr />
        </div>
